...
isSearchSearchPage:function(form)
{
 if (form)
 {
 var searchMsg = "\u641c\u7d22"; var oElement = form.elements["#ICSearch"]; var cElement = form.elements["#ICCancel"]; if (cElement != null) 
...
if (nOpenMode == "0")
 ...
 if (url.indexOf("?") != -1)
 url = url+"&"; else
 url = url+"?"; url = url+"ICModalJS=1&ICRCFModalJS=1&ICDoModeless=1"; if(url.search("IScript")!=-1)
...
function ShowMoreFacetValues(multiSelect, facetGbId, facetGridId, moreId, lessId) {
 ...
 elemFacetValuesGrid.style.width = nPrntWidth + "px"; elemFacetValuesGrid.style.height = "100%"; elemFacetValuesGrid.style.maxHeight = nGridHeight + "px"; elemFacetValuesGrid.setAttribute("origHeight", nOrigHeight); elemFacetValuesGrid.style.paddingBottom = "10px"; }
...
function ShowMoreTreeFacetValues(multiSelect, facetGbId, facetAreaId, moreId, lessId) {
 ...
 elemFacetValuesHtmlArea.style.width = nPrntWidth + "px"; elemFacetValuesHtmlArea.style.height = nGridHeight + "px"; }
}
...
function ShowLessTreeFacetValues(multiSelect, facetGbId, facetAreaId, moreId, lessId) {
 ...
 if (elemFacetValuesHtmlArea)
 elemFacetValuesHtmlArea.style.height = nHtmlAreaHeight + "px"; var elemHtmlArea = document.getElementById(facetAreaId); if (elemHtmlArea) {
...
PT_Search.prototype = {
 ...
 sUrl = sUrl.concat("&SEARCH_TEXT=" + encodeURIComponent(sKeyword)); LaunchURL(null, sUrl, 4); }
 },
 ...
 sUrl = this.aSrchGrps[i][1]; sUrl = sUrl.concat("?SEARCH_GROUP=" + sSrchCat + "&SEARCH_TEXT=" + sKeywords); LaunchURL(null, sUrl, 4); break; }
 }
 },

 GetSearchGroupsFromPortal: function(elemId, sPortalUrl, sCurrCompSrchCat) {
 var srchGrpObj = document.getElementById(elemId); if (!srchGrpObj)
 return; if (typeof(Storage) !== "undefined") {
 if (!sessionStorage.portalBsUrl) {
 sessionStorage.portalBsUrl = sPortalUrl + "/"; }
 }
...
var inside_empty = false;callbackOnDelay_empty= function(){ 
 ...
 delayobj.style.visibility="visible"; delayobj.style.display="block"; delayobj.innerHTML="<span> \u6b63\u5728\u5904\u7406\uff0c\u8bf7\u7a0d\u5019 </span>";  inside_empty=true; sessionStorage.setItem("globalInside_empty", inside_empty);   }
 }
 }
}
...
callback_end_empty= function(){
 ...
 delayobj.innerHTML="\u52a0\u8f7d\u5b8c\u6210"; setTimeout(initialize_empty,100);   }
 }
} 
...
function updatePageTitle()
{
  ...
  { 
  pageTitle = document.querySelector( ".PAPAGETITLE, .PSSRCHTITLE, h1");  if (typeof(pageTitle) != 'undefined' && pageTitle != null)
  {
   if(typeof(pageTitle.innerHTML) != 'undefined')
  {
   pageTitleString = pageTitle.innerHTML;  }
...
PT_Dialog.prototype = {
 ...
 this.processOptions(options, modObj, url);  var role = " role='dialog' "; if (modObj && modObj.bMsg) 
 role = " role='alertdialog' aria-describedby='alertmsg' "; role += "aria-labelledby='ptModTitle_" + this.idCount + "' "; if (modObj.bFolder) {
 ...
 modObj = null; return; }
 this.checkRemoveModeless(this.bModeless); if (modObj.bModeless) {
 ...
 sHtml += "<div id='ptModHeader_" + this.idCount + "'" + " class='psc_modal-pagetitle'>"; else
 ...
 sTitleHtml += "<span id='ptModTitle_" + this.idCount + "' class='PTPOPUP_TITLE'></span>"; if (modObj.bBack){
 ...
 sHtml += "<div id='ptModClose_" + this.idCount + "' class='ps_modal_close'><div class='ps_box-button psc_modal-close'><span class='ps-button-wrapper'>"; sHtml += "<a class='ps-button' role='button' alt='" + modalCloseAlt + "' id='ptModCloseLnk_" + this.idCount + "'"; if (isFModeLayout() && !isAccessibleLayout())
 ...
 modObj.style.visibility = "hidden"; modObj.innerHTML = sHtml; this.cObj.appendChild(modObj); modObj = MTop().document.getElementById(MTop().PTMOD_ + this.idCount); modObj.style.zIndex = this.zIndexBase + this.idCount * 10; if (modObj.bDivPopup) {
 ...
 oParentWin.modWin = modObj; this.resizeModalDialog(id); this.setAllMask(modObj, true); return modObj; }
 ...
 if (MTop().modlessId == -1) return false; if (typeof oParentWin == 'undefined' || oParentWin == null) 
 ...
 if (!modObj || !modObj.bMsg) return; if (isFModeLayout()) unhidePtWrapper(); modObj.oParentWin.modWin = null; if (modObj.oParentWin.winParent) {
 ...
 ptCommonObj2.tryFocus0(modObj.oParentWin.gFocusObj); modObj.innerHTML = ""; this.cObj.removeChild(modObj); if (typeof modObj.oParentWin.modalID != 'undefined' && modObj.oParentWin.modalID != null) {
 ...
 top.setAllMask(modObj, false); this.playMsg(); },
 ...
 var modObj = MTop().document.getElementById(MTop().PTMOD_ + id); if (!modObj) return true;  if (isFModeLayout() && isAccessibleLayout() || modObj.bViewAtt) unhidePtWrapper(); if (modObj.bDivPopup) { 
 if (typeof sessionStorage != "undefined") { sessionStorage.setItem("divPopupClosed", "1"); }
 ...
 if (isFModeLayout()) unhidePtWrapper(); var modObj = MTop().document.getElementById(MTop().PTMOD_ + id); if (!modObj) return; if (modObj.bDivPopup) {
 ...
 ptCommonObj2.hidePopupMask(top.window); if (bModeless && !isFModeLayout()) {
 ...
 w = otb.scrollWidth; h = otb.scrollHeight; if (oheader)
 ...
 h = oWin.document.body.scrollHeight; if (isFModeLayout() && browserInfoObj2.isFF && !modObj.bCustResize && h < 75) 
 h += 75;  if (isFModeLayout()){
 ...
 try {
 specModalPage = oWin.document.body.innerHTML.indexOf("javascript:DatePrompt_win") != -1 || oWin.document.body.innerHTML.indexOf("a name=\"PROMPT_XLAT1\"") != -1; } catch (e) {
 specModalPage = null; }
 }
 ...
 modObj.style.visibility = ""; if (modObj.bMsg && !isFModeLayout()) this.setMsgFocus(id);  if (isFModeLayout() && isAccessibleLayout()) hidePtWrapper();   var otb = MTop().document.getElementById("ptModTable_" + id);  if (modObj.nRight > 0)otb.style.right = modObj.nRight + 'px';  if (modObj.oParentWin.isFModeLayout()) {
 ...
 var modObjThis = MTop().document.getElementById(MTop().PTMOD_ + id); if (!modObjThis) continue; var oModParent = modObjThis.oParentWin; if (!oModParent) continue; var popMask = oModParent.document.getElementById('pt_modalMask'); if (!popMask) continue; if (bAutoClose)
...
positionPopup: function (modObj, otb, id) {
 if (!isFModeLayout() || !modObj.bPopup || modObj.bFullScreen || modObj.bCenter) return;  var oBody = document.getElementsByTagName("BODY")[0]; oBody.style.overflow = "hidden";  var fullWidth = ptCommonObj2.getViewportWidth() - 20; var fullHeight = ptCommonObj2.getViewportHeight() - 20; var rObj = this.getParentPopup(modObj); if (!rObj && modObj.nRCFLeft == -1) return; if (otb.clientWidth > fullWidth)
 otb.style.width = fullWidth-5 + 'px'; modObj.nRight = 0; var bResize = !modObj.bTypeAhead &&(otb.style.maxWidth != ''); var eArrowLeft = null; var eArrowRight = null; if (modObj.bTail) {
 ...
 return; var nRW = nParentRight - nParentLeft; var nLeft = nParentLeft - otb.clientWidth; var nRight = fullWidth - (nParentRight + otb.clientWidth); var nTop = nParentBottom - otb.clientHeight; var nBottom = fullHeight - (nParentBottom + otb.clientHeight)+20; var bRight = (nRight > 0 && nLeft < nRight || modObj.bCal && nRight>0) ? true : false; var bLeft = (nLeft > 0 && nLeft > nRight && !bRight) ? true : false;  var bUp = (nTop > 0 && nTop > nBottom) ? true : false; var bDown = (nBottom > 0 && nTop <= nBottom) ? true : false; var nLeftSp = nParentLeft; var nRightSp = fullWidth - nParentRight; if (fullWidth < 360 && nRightSp < 200 && nLeftSp < 200) {
 ...
 nLeft = nParentLeft; if (nLeft < 0) nLeft = nParentRight - otb.clientWidth; if ((nLeft + otb.clientWidth) > fullWidth) nLeft = fullWidth - otb.clientWidth -4; if (nLeft < 10) nLeft = 10; var nRight = fullWidth - (nLeft + otb.clientWidth); otb.style.left = nLeft + 'px'; if (!modObj.bTypeAhead) otb.style.right = nRight + 'px'; if (modObj.bTypeAhead) {
 var nMaxDown = Math.min((fullHeight - nParentBottom), otb.clientHeight); var nMaxTop = Math.min(nParentTop, otb.clientHeight); if (nMaxDown >= nMaxTop) {
 bDown = true; otb.style.height = nMaxDown + 'px'; } else {
 bDown = false; otb.style.height = nMaxTop + 'px'; }
 } 
 if (bDown) {
 ...
 otb.style.height = fullHeight - nTop + 'px'; eArrowRight.style.display = "none"; }
 else {
 ...
 nTop = nParentBottom - ((nParentBottom + otb.clientHeight) - fullHeight); if (nTop < 0) nTop = 0; otb.style.top = nTop + 'px';  if (fullHeight - nTop < otb.clientHeight)
 otb.style.height = fullHeight - nTop + 'px'; }
 }
 else
 {
 var nBottom = fullHeight-nParentTop; if (modObj.bAnm) addClass(modObj.firstChild, 'psc_easeb'); if (modObj.bTail) {
 eArrowRight.style.left = nParentLeft - nLeft + (nParentRight - nParentLeft)/2 + 'px'; eArrowRight.style.marginTop = '1px'; eArrowRight.style.display = "block"; eArrowLeft.style.display = "none"; if (nTop < 0) {
 nHeight = Math.min(fullHeight, otb.clientHeight, nParentTop); otb.style.height = (nHeight - (nTH + nBW))+ 'px'; }
 otb.style.bottom = nBottom + (nTH + nBW) + 'px'; }
 else {
 var nHeight = otb.clientHeight + nTop; if (modObj.bTypeAhead) {
 nTop = nTop - (nParentBottom - nParentTop); otb.style.top = nTop + 'px'; } else if (nTop < 0) {
 ...
 var owrap = otb.querySelector(".ps_mod_wrapc"); owrap.style.height = otb.clientHeight + "px"; }
 if (modObj.bRCFModal)
...
function closeLastModal(evt, target, oParentWin) {
 ...
 getPTDialog().doCloseModalDialog(MTop().modId);  if (isFModeLayout()) {
 var histElement; var histElementString = sessionStorage.getItem("pt_doback_popped_element_nui"); if (typeof histElementString != undefined && histElementString && typeof target != "undefined" && target != null && target &&(target.parentElement.id == "#ALERTYES" || target.parentElement.id == "#ICYes$span")) {
...
{
 var oWin = getLastModalWin(); if (oWin && typeof oWin.bLocalModal != "undefined")
...
function storeModalParentId(options) {
 ...
 if (typeof sessionStorage != "undefined" && typeof options != "undefined" && options) {
...
function closeMsg(obj,id) {
 var nRtn = getPTDialog().closeModalMsg(obj,id); setLocalModal(false); return nRtn;}
...
function post_to_url(TgtLoc, path, paramstring, method, strLabel,isRCFWin) {
 method = method || "POST";  var loader = new net2.ContentLoader(path, null, strLabel, "POST", 
 function()
 {
 var newurl = this.req.getResponseHeader("Location"); path = newurl; }, null, paramstring, "application/x-www-form-urlencoded", 1, 0, null, false);   if (path.indexOf("/s/") > -1) {
 var loader = new net2.ContentLoader(path, null, strLabel, "POST", 
 function()
 {
 var newurl = this.req.getResponseHeader("Location"); path = newurl; }, null, paramstring, "application/x-www-form-urlencoded", 1, 0, null, false); }
...
function OpenCrefInUniNav (Tgtobj,TgtLoc,bBulkAction,strBulkData, strLabel,nFluidComponent,isRCFWin){ 
 ...
 if ((typeof(nFluidComponent) != 'undefined') && nFluidComponent == 1)
 ...
 var UnsiteNamenew =""; var UnsiteNametemp = UnsiteName.lastIndexOf("_"); UnsiteNamenew = UnsiteName.substring(0,UnsiteNametemp); UnsiteNamenew = UnsiteNamenew + "_newwin"; URLArr[ai] = UnsiteNamenew; }
 }
 }
...
function getSrchRsltRelatedActions() {
 ...
 alert(srchRsltUrl + " " + "\u4e0d\u662f\u6709\u6548\u7684 URL \u683c\u5f0f\u3002"); return; }
...
function getOneSrchRsltRelatedActions(sSrchRsltUrl, raElemId, rowNum) {
 ...
 alert(sSrchRsltUrl + " " + "\u4e0d\u662f\u6709\u6548\u7684 URL \u683c\u5f0f\u3002"); return; }
 if (srchUrlHost.indexOf(':') < 0)
 srchUrlHost = srchUrlHost + ':80';   var bRemoteRA = true; var respText = "";  if ((localHost.length == srchUrlHost.length) && 
 ((localHost.toLowerCase()).indexOf(srchUrlHost.toLowerCase()) == 0))
 bRemoteRA = false; actionurl = origUrl; var tmpActionUrl = actionurl; var fldId = raElemId; var elemAction = fldId; if (bRemoteRA) {
...
function getAllRelatedActions() {
 window.status = "\u83b7\u53d6\u76f8\u5173\u64cd\u4f5c"; raFormatedString1 = ""; raFormatedString2 = ""; raFormatedString3 = ""; raFormatedString4 = ""; raFormatedString5 = ""; raFormatedString6 = ""; raFormatedString7 = ""; raFormatedString8 = ""; raFormatedString9 = ""; raFormatedString10 = ""; gSrchRaFldId = ""; try {
...
addNUIToBC : function () {
 ...
 var seps = ptUtil.getElemsByClass("pthnavhiearchysep", bcListUpdate, "li"); for (var i = 0; i < seps.length; i++) {
 ...
 var nIdx = bcList.children.length - 1; var eBcElement = bcList.children[nIdx];  while (ptUtil.isClassMember(eBcElement, "pthnavbarcref") || ptUtil.isClassMember(eBcElement, "pthnavbarfldr")) {
 ...
 alert("\u9762\u5305\u5c51\u66f4\u65b0\u51fd\u6570\u53d1\u751f\u9519\u8bef\u3002" + " " + inStr + ": " + e.message); } 
...
function UpdateHistory(label, keyData, userData, pageName, valid, userQueryString, nPost, bReturnToLastPage)
 ...
 if (testRec != null && top.document.getElementById('ptalPgltAreaFrame') != null) {
 var testRec = pt_history.pop(); if ( testRec.url == top.document.getElementById('ptalPgltAreaFrame').baseURI) { histRec.valid = 0; }
...
function AddToHistory(label, keyData, userData, pageName, stateNum, elemNum, classicURL, dashboard, appBcData, nPost, userQueryString, bReturnToLastPage) {
 ...
 if (url.indexOf("?") !== -1) {
 var urlParts = url.split("?"); url = urlParts[0]; var qVars = urlParts[1].split("&"), keyList = "", bFirstKey = true; for (var i = 0; i < qVars.length; i++) {
 if (qVars[i].toLowerCase().indexOf("page=") === -1 && qVars[i] != "") {
 ...
 var isFromBC = pt_history.isFromBC(); histRec = pt_history.lastValid(pageName, url); topRec = pt_history.pop(); if (topRec) {
 ...
 objBackText.innerHTML = "\u8fd4\u56de"; if (typeof objBackImg != "undefined" && objBackImg) {
...
function DoBack(bForm, bWarning) {
 var history = getHistoryObject(); if (history == null)
 return; var thisRec; if (isWorkCenter()) {
 ...
 if (thisRec == null || backRec == null)
 return; }
 history.save();    var bPost = backRec.isPost && (thisRec.url.split("?")[0] == backRec.url.split("?")[0]); if (!bPost ||
 (backNavigation.classicBackButton.isUsed() && thisRec.url.split("?")[0] == backRec.url.split("?")[0] && backNavigation.isClassicSearch(backRec)) ||
 backNavigation.isNuiSearch(backRec)) {
 ...
 if ((typeof bWarning == "undefined") || bWarning) {
 DoURLWarning(sUrl, thisRec, backRec); } else {
 if (backRec && backRec.isWorkCenter) {
 top.location = sUrl; } else {
 window.location.href = sUrl; }
 }
 ...
 genUserPostParams(backRec, bForm); var sEvent = "document." + bForm + ".ICPanelName.value = '" + backRec.pageName + "';\n submitAction_" + bForm + "(" + bForm + " ,'#ICBack');"; if (!isFModeLayout()) {
 
 sEvent = "var oContentWindow = window.document.getElementById('ptifrmtgtframe').contentWindow;\n"
...
function DoURLWarning(url, histElement, backRec)
{
 ...
 bChanged = checkFrameChanged(window); var agTarget = typeof document.getElementById("ICAGTarget") != 'undefined' && document.getElementById("ICAGTarget"); var agChanged = agTarget ? (typeof document.getElementById("ICUOW") != 'undefined' && document.getElementById("ICUOW") != null && document.getElementById("ICUOW").value != "N" ? true : false) : false; if (bChanged || agChanged) {

 
 var bValidHistElement = typeof histElement != 'undefined' && histElement; bValidHistElement ? sessionStorage.setItem("pt_doback_popped_element_nui", JSON.stringify(histElement)) : sessionStorage.removeItem("pt_doback_popped_element_nui"); var sJS = 'javascript:window.location.href = "' + url+'"'; psConfirmSW("", sJS, window); }
 else if (typeof backRec != 'undefind' && backRec && backRec.isWorkCenter) {
 top.location = url; }
 else
...
function fixUpBackButton() {
 ...
 objBackText.innerHTML = "\u8fd4\u56de"; var objBackImg = backBtn.querySelector(".ps-img"); if (typeof objBackImg != "undefined" && objBackImg) {
...
function updClassicHistory() {
 ...
 if (n.url.split("/")[8].split("?")[0] == url.split("/")[8].split("?")[0]) { n.cURL = url; }
 } else {
 n.cURL = n.url; }
...
function PT_Stack() 
{
 ...
 }; this.lastValid = function(pageName, url) {
...
function PnWindowSubscribe() {
 ...
 if (!IsPNEnabled()) return;   var subscriptionFilter = SetSubscriptionFilter("PUSHNOTIFICATIONWINDOW", true); subscriptionFilter.subscribeOnContentNodes = false; AddSubscriptionFilter(subscriptionFilter); SubscribeCollection("PUSHNOTIFICATIONWINDOW", UpdatePNUI); pm.wsIsPNW = 1;}
...
var backNavigation = {
 ...
 if (isFModeLayout()) { 
 cookieObject.setCookie(this.psback, this.setCookieValue(setUrl(histRec), histRec.label, this.origin, getLayout()), this.expireDuration);  } else { 
 ...
 cookieObject.setCookie(this.psback, this.setCookieValue(url, histRec.label, this.origin, setTimeout(function(){getLayout()}, 0)), this.expireDuration); } else { 
 cookieObject.setCookie(this.psback, this.setCookieValue(document.location.href, setLabel(), this.origin, setTimeout(function(){getLayout()}, 0)), this.expireDuration); }
 }
 ...
 var backRec = JSON.parse(event.data), pt_history = getHistoryObject(), backBtn = top.document.querySelector("#PT_WORK_PT_BUTTON_BACK"); if (!backBtn) { 
 if (isFModeLayout()) {
 ...
 if (backRec && pt_history && backBtn && !backNavigation.isInHistory(backRec)) {
 ...
 backBtnTxt.innerHTML = "\u8fd4\u56de"; if (typeof objBackImg != "undefined" && objBackImg) {
 backNavigation.setAltText(objBackImg); }
 }
 pt_history.save(); }
 ...
 try {
 if (typeof refUrl == "undefined" || !refUrl || typeof curUrl == "undefined" || !curUrl) return false;  var refUrlNode = getNodeName(refUrl), bNodeFound = false; if (refUrl.search("cmd=") < 0) {
 ...
 var rcFrame = top.document.querySelector(".psc_groupletiframe"); if (rcFrame) { return false; }
...
setAriaLabel : function(inObj, histRecLabel, histRec, histData) {
 var szType = "\u8fd4\u56de"; if (typeof bGuidedAG != "undefined" && bGuidedAG) { szType = "\u9000\u51fa"; }
 ...
 var dropDownCntnr = top.document.querySelector("#pthdr2container");  var divEl00 = document.createElement("div"); divEl00.setAttribute("id", "classicbackbuttonlayout"); var divEl01 = document.createElement("div"); divEl01.setAttribute("class", "ps_box-button ps_button_backnav psc_header-all"); divEl01.setAttribute("id", "classicbackbuttoncontainer"); var spanEl = document.createElement("span"); spanEl.setAttribute("class", "ps-button-wrapper"); spanEl.setAttribute("title", "Back"); var ancEl = document.createElement("a"); ancEl.setAttribute("id", "PT_WORK_PT_BUTTON_BACK"); ancEl.setAttribute("class", "ps-button"); ancEl.setAttribute("role", "button"); var href = "javascript:DoBack('xyz')";  if (typeof winName != 'undefined' && winName) { href = "javascript:DoBack('" + winName + "')"; }
 ancEl.setAttribute("href", href); var imgEl = document.createElement("img"); imgEl.setAttribute("src", "/cs/CSPRD/cache/PT_NUI_BACK_PRIM_IMG_1.svg"); imgEl.setAttribute("name", "PT_WORK_PT_BUTTON_BACK$IMG"); imgEl.setAttribute("class", "ps-img"); backNavigation.setAltText(imgEl); var spanTxtEl = document.createElement("span"); spanTxtEl.setAttribute("class", "ps-text"); ancEl.appendChild(imgEl); ancEl.appendChild(spanTxtEl); spanEl.appendChild(ancEl); divEl01.appendChild(spanEl); divEl00.appendChild(divEl01); dropDownCntnr.insertBefore(divEl00, dropDownCntnr.children[0]);  }
 } 
