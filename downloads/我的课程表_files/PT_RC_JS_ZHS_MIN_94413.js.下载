...
else if(key == 38) 
{
 ...
 if (expandedSize == "0")
 expandedSize = ptrc.RC_COL_MAXSIZE; tcf.cols = expandedSize; var oldExpander = rcf.document.getElementById("ptrccollapse_expand"); if (oldExpander) oldExpander.parentNode.parentNode.style.display = "none"; rcf.document.getElementById("ptrcServiceFrame").className = "SFRCPosition"; imgBar.className = "";  imgBar.parentNode.title = '\u6298\u53e0' + ' ' + '\u76f8\u5173\u5185\u5bb9'; } else if (this.expandedRCCol == null || this.expandedRCCol == this.RC_MIN) {
 tcf.cols = ptrc.RC_COL_MAXSIZE; imgBar.className = ""; imgBar.parentNode.title = '\u6298\u53e0' + ' ' + '\u76f8\u5173\u5185\u5bb9'; } else {
 ...
 ifr = ifr.getElementsByTagName("iframe")[0]; ifr.title = serviceName + " \u76f8\u5173\u5185\u5bb9";  ifr.setAttribute("serviceId", serviceId); }
 },
 ...
 rcBoby.style.display = "block"; titleStr = titleStr.replace("\u5c55\u5f00","\u6298\u53e0"); expCollImg.title = titleStr; this.bRCExpanded = true;    height = 0; ptUtil.swapClass(expCollImg,"ptrccollapse","ptrcexpand"); if (!this.bServiceLoadedInFrame)
 this.onRefresh();  } else {
 this.bRCExpanded = false; rcBoby.style.display = "none"; titleStr = titleStr.replace("\u6298\u53e0","\u5c55\u5f00"); expCollImg.title = titleStr; ptUtil.swapClass(expCollImg,"ptrcexpand","ptrccollapse");  var titleBar = ptUtil.id("ptrctitlebar"); var TCSpacer = ptUtil.id("ptrcTCSpacer"); height = titleBar.offsetHeight + TCSpacer.offsetHeight; }
