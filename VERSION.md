...
## v0.3.6 (2025-01-13) - 简化日历生成，专注课程导入 📚

### 核心简化
- 🎯 **专注课程** - 暂时移除特殊事件和节假日处理，优先完成基础课程导入
- 📉 **减小文件** - 大幅减少生成的ICS文件大小，提高导入成功率
- 🔧 **简化逻辑** - 使用`generateSimpleClassDates`函数，只处理基本的课程重复
- 🧹 **移除调试** - 清理多余的调试信息，提高性能

### 修复内容
- ✅ **时间格式** - 修复时间显示为00000的问题
- ✅ **换行符** - 使用正确的`\\n`格式而不是`\\\\n`
- ✅ **文件大小** - 从6万多行减少到合理大小
- ✅ **导入兼容性** - 参考0.3.2.ics的成功格式

### 功能调整
- 📋 **基础课程** - 确保所有课程在整个学期中正确重复
- 🚫 **暂停特殊事件** - 不生成校园活动、节假日等特殊事件
- 🚫 **暂停调休处理** - 不处理调休和补课逻辑
- ⚡ **性能优化** - 更快的生成速度和更小的文件

### 使用优先级
1. **首要目标** - 确保基础课程能够成功导入各大日历应用
2. **次要目标** - 在基础功能稳定后，再逐步添加高级功能
3. **文件大小** - 控制在合理范围内，确保导入成功
