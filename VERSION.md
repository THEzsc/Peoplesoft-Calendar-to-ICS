...
## v0.5.1 (2026-01-05) - 多节识别与补课事件优化 ✅

### 课程解析
- ✅ **同日多节识别** - 拆分单元格内多个时间段，避免只识别一节
- 🧩 **多时间段解析** - 同一行出现多段时间时按节次拆分

### 行事历与调休
- ✅ **补课独立事件** - 取消 RDATE，补课改为单独 VEVENT

## v0.5.0 (2026-01-04) - 结构化解析与调休完善 ✅

### 课程解析
- 🧱 **结构化定位** - 基于课程容器与会议表格解析，适配中英文界面
- 🕒 **英文缩写扩展** - 支持 Mo/Tu/We/Th/Fr/Sa/Su + AM/PM + 24小时制
- 🧩 **行内继承** - 处理多行记录时自动继承组件/教室/讲师

### 行事历与调休
- 🚫 **节假日停课** - 假期统一用 EXDATE 排除
- ➕ **调休补课** - 用 RDATE 添加补班（如 5/1 → 5/9）
- 📅 **春夏学期更新** - 行事历与全日重要事件调整

### ICS 输出
- ✅ **最后一周修复** - RRULE 计数按实际发生次数
- 🎉 **特殊事件统一** - 读取 specialEvents 配置生成全日事件
