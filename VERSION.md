...
## v0.3.3 (2025-01-13) - 恢复RRULE机制修复显示问题 🔄

### 核心修复
- ✅ **恢复RRULE重复规则** - 修复v0.3.2中课程只显示一次的关键问题
- ✅ **整学期课程显示** - 使用RRULE确保所有课程在整个学期中重复显示
- ✅ **EXDATE异常处理** - 正确处理节假日和停课日期的异常
- ✅ **描述换行修复** - 修复ICS描述字段的换行显示问题

### RRULE实现机制
- 📅 **智能重复规则** - 为每个上课日自动生成FREQ=WEEKLY规则
- 🚫 **异常日期处理** - 使用EXDATE排除节假日和停课日期
- 📆 **学期范围控制** - UNTIL参数确保课程不超出学期范围
- 🔄 **补课日支持** - 保持对调休补课日的支持

### 兼容性保持
- 📱 **Outlook兼容** - 保持v0.3.2中的兼容性修复
- 🎯 **标准格式** - 继续使用"课程代码 - 组件"的友好格式
- 📝 **丰富描述** - 保持详细的课程信息描述
- 🏷️ **分类标签** - 维持事件分类功能

### 问题解决
- 🔧 **单次显示问题** - 从单个事件改回RRULE重复机制
- 📊 **完整学期覆盖** - 确保课程在整个学期中正确显示
- 📖 **描述格式** - 修复换行符在各日历应用中的显示
- ⏰ **时间准确性** - 保持精确的上课时间和时区处理
