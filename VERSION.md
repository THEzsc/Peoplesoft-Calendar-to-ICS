...
## v0.3.9 (2025-01-13) - 智能检测单双周课程模式 🔍

### 核心改进
- 🔍 **模式检测** - 智能分析课程时间跨度，自动检测单双周和不规则上课模式
- 📅 **RRULE优化** - 对规律课程使用`RRULE:FREQ=WEEKLY;INTERVAL=2`（单双周）
- 📋 **RDATE支持** - 对不规则课程使用`RDATE`精确指定上课日期
- 🎯 **精确匹配** - 基于实际课程日期间隔进行模式识别

### 单双周检测算法
1. **分析间隔** - 计算连续上课日期之间的周数间隔
2. **模式识别** - 检测是否为一致的2周间隔（单双周）
3. **智能选择** - 规律模式用RRULE，不规律模式用RDATE
4. **精确日期** - 使用实际首次上课日期作为起始点

### 支持的课程模式
- ✅ **每周课程** - `RRULE:FREQ=WEEKLY;COUNT=16;BYDAY=MO`
- ✅ **单双周课程** - `RRULE:FREQ=WEEKLY;INTERVAL=2;COUNT=8;BYDAY=MO`  
- ✅ **不规则课程** - `RDATE;TZID=Asia/Shanghai:20250915T140000,20250929T140000,...`
...
- 20/10/2025 (第5周) → 间隔2周
- 03/11/2025 (第7周) → 间隔2周
→ 检测为单双周模式，使用INTERVAL=2
```

### 调试信息
- 📊 **间隔分析** - 控制台显示"课程间隔周数"数组
- 🔍 **模式识别** - 显示检测到的模式类型和参数
- 📋 **生成方式** - 显示使用RRULE还是RDATE模式
