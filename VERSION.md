...
## v0.3.9 (2025-01-13) - 增强按钮注入和调试功能 🔍

### 核心改进
- 🔍 **调试按钮** - 添加固定位置的红色调试按钮，总是可见
- 📊 **详细诊断** - 点击调试按钮显示脚本状态、页面信息、环境检测
- 🔄 **重试机制** - 增加5秒延迟重试，提高按钮注入成功率
- 📋 **日志增强** - 详细的控制台日志，便于问题排查

### 调试功能
- **固定调试按钮**: 页面右上角红色"🔧 PS Calendar Debug"按钮
- **环境检测**: 显示脚本版本、URL、Tampermonkey版本等信息
- **课程表检测**: 实时显示是否找到课程表元素
- **强制导出**: 即使主按钮不可见也能导出ICS文件

### 解决分享问题
- **可见性保证**: 调试按钮使用`position: fixed`和最高z-index
- **兼容性检查**: 检测Tampermonkey环境和浏览器兼容性
- **故障排除**: 创建详细的故障排除指南文档

### 按钮注入机制
1. **主按钮**: 智能检测课程表页面并注入"导出 ICS"按钮
2. **调试按钮**: 强制注入，总是可见，用于诊断问题
3. **重试机制**: 5秒后自动重试注入，处理页面动态加载
4. **iframe处理**: 监听PeopleSoft的iframe加载

### 故障排除文档
创建了`TROUBLESHOOTING.md`文档，包含：
- 常见问题解决方案
- Tampermonkey设置检查
- 浏览器兼容性说明
- 分享给他人的注意事项
